...
+ (NSString*)passwordToEmoji:(NSString*)password {
  ...
  static unsigned int alphabet_size[] = {ARRAY_SIZE(smile), ARRAY_SIZE(gesture), ARRAY_SIZE(animal),
                                         ARRAY_SIZE(food), ARRAY_SIZE(object)};

  uint64_t fingerprint = digest[4] | (digest[5] << 8) | (digest[6] << 16) | (digest[7] << 24);
  fingerprint <<= 32;
  fingerprint |= digest[0] | (digest[1] << 8) | (digest[2] << 16) | (digest[3] << 24);
...
+ (void)passwordToAESAndMACKey:(NSString*)password
  ...
  dispatch_async(queue, ^{
    ...
    scrypt(&state, (const uint8_t*)origin.UTF8String, origin.length, (const uint8_t*)kScryptAES,
           sizeof(kScryptAES) - 1, key_data, sizeof(key_data));
    scrypt_state_destroy(&state);

    __block NSData* aes_key = [NSData dataWithBytes:key_data length:kCryptorKeySize];
    __block NSData* mac_key =
        [NSData dataWithBytes:key_data + kCryptorKeySize length:kCryptorMacKeySize];
...
+ (void)passwordFromMaster:(NSString*)master
  ...
  dispatch_async(queue, ^{
    ...
    char tmp[1024];
    if (revision <= 1) {
      snprintf(tmp, sizeof(tmp), "%s/%s", domainCopy.UTF8String, loginCopy.UTF8String);
    } else {
      snprintf(tmp, sizeof(tmp), "%s/%s#%d", domainCopy.UTF8String, loginCopy.UTF8String, revision);
    }
